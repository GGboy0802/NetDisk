...
//发送分享文件请求 ：包含 谁 分享 什么目录下面的 哪些文件( 文件id 数组 )  分享时间
...
////获取上传中pos
//请求
#define _DEF_PACK_GETUPLOADPOS_RQ     (_DEF_PACK_BASE + 30)
#define _DEF_PACK_GETUPLOADPOS_RS     (_DEF_PACK_BASE + 31)

struct STRU_GETUPLOADPOS_RQ
{
    STRU_GETUPLOADPOS_RQ():type(_DEF_PACK_GETUPLOADPOS_RQ){
        userid = 0;
        timestamp = 0;
        fileid = 0;
        memset( dir , 0 , sizeof(dir));
        memset( f_md5 , 0 , sizeof(f_md5));
    }
    ...
    int timestamp;
    int fileid;
    char f_md5[_MAX_PATH_SIZE];
    char dir[_MAX_PATH_SIZE];
};

struct STRU_GETUPLOADPOS_RS
{
    STRU_GETUPLOADPOS_RS():type(_DEF_PACK_GETUPLOADPOS_RS){
        fileid = 0;
        timestamp = 0;
        pos = 0;
        //memset( dir , 0 , sizeof(dir));
